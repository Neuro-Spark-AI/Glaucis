...
def _flash_attention_kernel_mhpt(
  ...
  def _one_chunk(slice_k):
    # One online-softmax update per bkv_compute chunk. The prior V1 tiling split
    # each chunk into bkv_compute_in=256 micro-steps, each running its own
    # max / exp(qk-m_next) / exp(m_prev-m_next) rescale. Rescaling and the running
    # max are done once over the whole chunk here instead of per micro-step: the
    # exp(m_prev-m_next) rescale (alpha) and the maximum drop from two per chunk to
    # one, and s_curr = exp(qk - m_next) becomes a single wide vector op over the
    # full chunk instead of two narrow ones. Online softmax is exact for any chunk
    # granularity, so widening the chunk from 256 to the full bkv_compute leaves the
    # result identical (max, then exp, then normalize is associative over the rows).
