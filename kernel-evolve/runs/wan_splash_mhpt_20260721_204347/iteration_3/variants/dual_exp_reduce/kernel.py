...
def _flash_attention_kernel_mhpt(
  ...
  def _one_chunk(slice_k):
    ...
    k_chunk = k_ref[:, slice_k, :]           # [hpt, bkv_compute, d]
    ...
    # --- Merged online-softmax over the whole BKV_COMPUTE chunk ---
    # The dual_mxu base split each bkv_compute=512 chunk into two
    # bkv_compute_in=256 micro-steps, each with its own max / exp2(qk - m) /
    # sum / alpha=exp2(m_prev - m_next) rescale of l and o. Merging the two
    # adjacent micro-steps into a single streaming-softmax step over the full
    # chunk is mathematically exact — online softmax is associative, so folding
    # sub-blocks j and j+1 with a shared running max gives the same (m, l, o) as
    # processing them in sequence. Doing it in one shot cuts the per-chunk EUP
    # work: one running max instead of two, one exp2 over the full
    # [hpt, bkv_compute, bq] block instead of two 256-wide ones, one alpha exp2
    # instead of two, and one l/o rescale instead of two. The single wide exp2
    # and the single full-chunk PV dot also pack better than two half-width ops.
    m_curr = qk.max(axis=1)[:, None, :]                    # [hpt, 1, bq]
    m_next = jnp.maximum(m_prev, m_curr)                   # [hpt, NUM_SUBLANES, bq]
    s_curr = exp(qk - m_next[:, 0:1, :])                   # [hpt, bkv_compute, bq]
    ...
    o_curr = lax.dot_general(
        v_chunk, s_curr.astype(q_ref.dtype),
