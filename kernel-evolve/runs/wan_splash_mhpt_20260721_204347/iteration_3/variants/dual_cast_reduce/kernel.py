...
# dual_cast_reduce direction: trim dtype-cast / operand-prep overhead around the
# two head-batched dots, keeping the matmul math and the softmax result byte-for-
# byte the same.
#   1. QKᵀ operands enter the loop already bf16 (q pre-scaled+cast in
#      optimized_compute, k comes in bf16), so no cast happens on that dot.
#   2. The softmax probabilities are cast to bf16 exactly once, bound to a named
#      value right before the PV dot — no repeated .astype of the same tensor.
#   3. The o-rescale epilogue drops jnp.tile: m/l scratch keep every sublane row
#      as an identical replica, so row 0 is the true 1/l. Broadcasting that single
#      row over head_dim_v gives the same product as tiling a [head_dim_v, bq]
#      copy, without materializing it, and vectorizes the whole head-tile at once.
...
def _flash_attention_kernel_mhpt(
  ...
  def _one_chunk(slice_k):
    # All heads in the tile at once. Shapes carry an explicit leading head axis
    # so both dots below are single batched 3D dot_generals (batch dim = head),
    # letting the scheduler use both MXUs instead of serializing per head.
    q_all = q_ref[...]                       # [hpt, bq, d]         (bf16)
    k_chunk = k_ref[:, slice_k, :]           # [hpt, bkv_compute, d]  (bf16)
    v_chunk = v_ref[:, slice_k, :]           # [hpt, bkv_compute, d]

    # Batched QKᵀ across the head-tile -> [hpt, bkv_compute, bq].
    # Both operands are already bf16, so no operand cast happens on this dot.
    ...
    # --- V1 VPU register tiling (now vectorized over the head axis) ---
    step = bkv_compute_in
    for i in range(0, qk.shape[1], step):
      ...
      s_curr = exp(qk_slice - m_next[:, 0:1, :])            # [hpt, step, bq]  (f32)
      l_curr = s_curr.sum(axis=1, keepdims=True)            # [hpt, 1, bq]
      alpha = exp(m_prev - m_next)                          # [hpt, NUM_SUBLANES, bq]
      l_next = l_curr + alpha * l_prev
      # Single bf16 cast of the softmax probabilities, right before the PV dot.
      s_curr_bf = s_curr.astype(q_ref.dtype)               # [hpt, step, bq]  (bf16)
      # Batched PV across the head-tile -> [hpt, head_dim_v, bq].
      o_curr = lax.dot_general(
          v_chunk[:, i:i + step, :], s_curr_bf,
  ...
  @pl.when(j == grid_width - 1)
  def end():
    # Rescale the whole head-tile at once. Every sublane row of l is an identical
    # replica (m/l are carried sublane-broadcast), so row 0 is the true 1/l.
    # Broadcasting it over head_dim_v gives the same product as tiling a
    # [head_dim_v, bq] copy per head, without materializing the tile.
    l_inv = 1.0 / l_scratch_ref[:, 0:1, :]                  # [hpt, 1, bq]
