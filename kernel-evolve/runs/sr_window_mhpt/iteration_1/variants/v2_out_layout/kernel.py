...
def _window_sparse_kernel_mhpt(
  ...
  def _one_chunk(chunk_idx):
    k_start = j * bkv + chunk_idx * bkv_compute
    # Frame-aligned blocks put the whole chunk inside one frame of one shard.
    k_frame = (k_start % q_seq_len) // LOCAL_FRAME
  ...
  def end():
    for h_local in range(heads_per_tile):
      ...
      # Output layout is [H, S, D]: transpose the [Dv, bq] accumulator once per block.
      o_ref[h_local] = jnp.transpose(
          (o_scratch_ref[h_local] * l_inv), (1, 0)).astype(o_ref.dtype)
...
def optimized_compute(H=5, SQ=39600, SKV=79200, D=128):
  ...
  out = _window_sparse_forward(
      ...
      vmem_limit_bytes=VMEM_LIMIT_BYTES)  # [H, SQ, D]
  return out
# EVOLVE-BLOCK-END
