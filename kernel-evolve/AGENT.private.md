...
### [F007 / S003] wan_splash_mhpt R3 — micro-opts neutral, kernel converged on this structure
- On the dual_mxu base (2.327x): bkv_compute=1024 → 2.304x, cast/tile reduce → 2.300x, exp/rescale merge → 2.294x. All within ~1% noise, none beat dual_mxu.
- double_buffer_kv failed a 2nd time (make_async_copy manual 2-slot, after emit_pipeline in R2 also threw). Manual KV double-buffering on this mhpt kernel is correctness/compile-fragile on jax 0.11; not worth more attempts without deeper Pallas debugging.
- Converged: block sizes fixed (e2e-optimal), both MXUs already balanced (dual_ratio ~1.0 after the profiler fix), micro-opts neutral. Remaining hard lever is head_dim=128 < 256 MXU-contraction underfill (structural; can't pack 2 heads' head_dim without changing the math). dual_mxu (head-batched 3D dot, +12.8% at production [5,86016,128]) is the deliverable win.
